...
!function(t,e,a,s){function i(e,s){this.w=t(a),this.el=t(e),this.options=t.extend({},l,s),this.init()}var o="ontouchstart"in a,n=function(){var t=a.createElement("div"),s=a.documentElement;if(!("pointerEvents"in t.style))return!1;t.style.pointerEvents="auto",t.style.pointerEvents="x",s.appendChild(t);var i=e.getComputedStyle&&"auto"===e.getComputedStyle(t,"").pointerEvents;return s.removeChild(t),!!i}(),l={listNodeName:"ol",itemNodeName:"li",rootClass:"dd",listClass:"dd-list",itemClass:"dd-item",dragClass:"dd-dragel",handleClass:"dd-handle",collapsedClass:"dd-collapsed",placeClass:"dd-placeholder",noDragClass:"dd-nodrag",emptyClass:"dd-empty",expandBtnHTML:'<button data-action="expand" type="button">Expand</button>',collapseBtnHTML:'<button data-action="collapse" type="button">Collapse</button>',group:0,maxDepth:5,threshold:20};i.prototype={init:function(){var a=this;a.reset(),a.el.data("nestable-group",this.options.group),a.placeEl=t('<div class="'+a.options.placeClass+'"/>'),t.each(this.el.find(a.options.itemNodeName),function(e,s){a.setParent(t(s))}),a.el.on("click","button",function(e){if(!a.dragEl){var s=t(e.currentTarget),i=s.data("action"),o=s.parent(a.options.itemNodeName);"collapse"===i&&a.collapseItem(o),"expand"===i&&a.expandItem(o)}});var s=function(e){var s=t(e.target);if(!s.hasClass(a.options.handleClass)){if(s.closest("."+a.options.noDragClass).length)return;s=s.closest("."+a.options.handleClass)}s.length&&!a.dragEl&&(a.isTouch=/^touch/.test(e.type),a.isTouch&&1!==e.touches.length||(e.preventDefault(),a.dragStart(e.touches?e.touches[0]:e)))},i=function(t){a.dragEl&&(t.preventDefault(),a.dragMove(t.touches?t.touches[0]:t))},n=function(t){a.dragEl&&(t.preventDefault(),a.dragStop(t.touches?t.touches[0]:t))};o&&(a.el[0].addEventListener("touchstart",s,!1),e.addEventListener("touchmove",i,!1),e.addEventListener("touchend",n,!1),e.addEventListener("touchcancel",n,!1)),a.el.on("mousedown",s),a.w.on("mousemove",i),a.w.on("mouseup",n)},serialize:function(){var e,a=0,s=this;return step=function(e,a){var i=[],o=e.children(s.options.itemNodeName);return o.each(function(){var e=t(this),o=t.extend({},e.data()),n=e.children(s.options.listNodeName);n.length&&(o.children=step(n,a+1)),i.push(o)}),i},e=step(s.el.find(s.options.listNodeName).first(),a)},serialise:function(){return this.serialize()},reset:function(){this.mouse={offsetX:0,offsetY:0,startX:0,startY:0,lastX:0,lastY:0,nowX:0,nowY:0,distX:0,distY:0,dirAx:0,dirX:0,dirY:0,lastDirX:0,lastDirY:0,distAxX:0,distAxY:0},this.isTouch=!1,this.moving=!1,this.dragEl=null,this.dragRootEl=null,this.dragDepth=0,this.hasNewRoot=!1,this.pointEl=null},expandItem:function(t){t.removeClass(this.options.collapsedClass),t.children('[data-action="expand"]').hide(),t.children('[data-action="collapse"]').show(),t.children(this.options.listNodeName).show()},collapseItem:function(t){var e=t.children(this.options.listNodeName);e.length&&(t.addClass(this.options.collapsedClass),t.children('[data-action="collapse"]').hide(),t.children('[data-action="expand"]').show(),t.children(this.options.listNodeName).hide())},expandAll:function(){var e=this;e.el.find(e.options.itemNodeName).each(function(){e.expandItem(t(this))})},collapseAll:function(){var e=this;e.el.find(e.options.itemNodeName).each(function(){e.collapseItem(t(this))})},setParent:function(e){e.children(this.options.listNodeName).length&&(e.prepend(t(this.options.expandBtnHTML)),e.prepend(t(this.options.collapseBtnHTML))),e.children('[data-action="expand"]').hide()},unsetParent:function(t){t.removeClass(this.options.collapsedClass),t.children("[data-action]").remove(),t.children(this.options.listNodeName).remove()},dragStart:function(e){var i=this.mouse,o=t(e.target),n=o.closest(this.options.itemNodeName);this.placeEl.css("height",n.height()),i.offsetX=e.offsetX!==s?e.offsetX:e.pageX-o.offset().left,i.offsetY=e.offsetY!==s?e.offsetY:e.pageY-o.offset().top,i.startX=i.lastX=e.pageX,i.startY=i.lastY=e.pageY,this.dragRootEl=this.el,this.dragEl=t(a.createElement(this.options.listNodeName)).addClass(this.options.listClass+" "+this.options.dragClass),this.dragEl.css("width",n.width()),n.after(this.placeEl),n[0].parentNode.removeChild(n[0]),n.appendTo(this.dragEl),t(a.body).append(this.dragEl),this.dragEl.css({left:e.pageX-i.offsetX,top:e.pageY-i.offsetY});var l,d,r=this.dragEl.find(this.options.itemNodeName);for(l=0;l<r.length;l++)d=t(r[l]).parents(this.options.listNodeName).length,d>this.dragDepth&&(this.dragDepth=d)},dragStop:function(t){var e=this.dragEl.children(this.options.itemNodeName).first();e[0].parentNode.removeChild(e[0]),this.placeEl.replaceWith(e),this.dragEl.remove(),this.el.trigger("change"),this.hasNewRoot&&this.dragRootEl.trigger("change"),this.reset()},dragMove:function(s){var i,o,l,d,r,c=this.options,p=this.mouse;this.dragEl.css({left:s.pageX-p.offsetX,top:s.pageY-p.offsetY}),p.lastX=p.nowX,p.lastY=p.nowY,p.nowX=s.pageX,p.nowY=s.pageY,p.distX=p.nowX-p.lastX,p.distY=p.nowY-p.lastY,p.lastDirX=p.dirX,p.lastDirY=p.dirY,p.dirX=0===p.distX?0:p.distX>0?1:-1,p.dirY=0===p.distY?0:p.distY>0?1:-1;var h=Math.abs(p.distX)>Math.abs(p.distY)?1:0;if(!p.moving)return p.dirAx=h,void(p.moving=!0);p.dirAx!==h?(p.distAxX=0,p.distAxY=0):(p.distAxX+=Math.abs(p.distX),0!==p.dirX&&p.dirX!==p.lastDirX&&(p.distAxX=0),p.distAxY+=Math.abs(p.distY),0!==p.dirY&&p.dirY!==p.lastDirY&&(p.distAxY=0)),p.dirAx=h,p.dirAx&&p.distAxX>=c.threshold&&(p.distAxX=0,l=this.placeEl.prev(c.itemNodeName),p.distX>0&&l.length&&!l.hasClass(c.collapsedClass)&&(i=l.find(c.listNodeName).last(),r=this.placeEl.parents(c.listNodeName).length,r+this.dragDepth<=c.maxDepth&&(i.length?(i=l.children(c.listNodeName).last(),i.append(this.placeEl)):(i=t("<"+c.listNodeName+"/>").addClass(c.listClass),i.append(this.placeEl),l.append(i),this.setParent(l)))),p.distX<0&&(d=this.placeEl.next(c.itemNodeName),d.length||(o=this.placeEl.parent(),this.placeEl.closest(c.itemNodeName).after(this.placeEl),o.children().length||this.unsetParent(o.parent()))));var u=!1;if(n||(this.dragEl[0].style.visibility="hidden"),this.pointEl=t(a.elementFromPoint(s.pageX-a.body.scrollLeft,s.pageY-(e.pageYOffset||a.documentElement.scrollTop))),n||(this.dragEl[0].style.visibility="visible"),this.pointEl.hasClass(c.handleClass)&&(this.pointEl=this.pointEl.parent(c.itemNodeName)),this.pointEl.hasClass(c.emptyClass))u=!0;else if(!this.pointEl.length||!this.pointEl.hasClass(c.itemClass))return;var m=this.pointEl.closest("."+c.rootClass),f=this.dragRootEl.data("nestable-id")!==m.data("nestable-id");if(!p.dirAx||f||u){if(f&&c.group!==m.data("nestable-group"))return;if(r=this.dragDepth-1+this.pointEl.parents(c.listNodeName).length,r>c.maxDepth)return;var g=s.pageY<this.pointEl.offset().top+this.pointEl.height()/2;o=this.placeEl.parent(),u?(i=t(a.createElement(c.listNodeName)).addClass(c.listClass),i.append(this.placeEl),this.pointEl.replaceWith(i)):g?this.pointEl.before(this.placeEl):this.pointEl.after(this.placeEl),o.children().length||this.unsetParent(o.parent()),this.dragRootEl.find(c.itemNodeName).length||this.dragRootEl.append('<div class="'+c.emptyClass+'"/>'),f&&(this.dragRootEl=m,this.hasNewRoot=this.el[0]!==this.dragRootEl[0])}}},t.fn.nestable=function(e){var a=this,s=this;return a.each(function(){var a=t(this).data("nestable");a?"string"==typeof e&&"function"==typeof a[e]&&(s=a[e]()):(t(this).data("nestable",new i(this,e)),t(this).data("nestable-id",(new Date).getTime()))}),s||a}}(window.jQuery||window.Zepto,window,document);var _ignorehash,admin={wysiwyg:{add:function(t,e){e=e||{};var a={selector:"#"+t,content_css:"./sites/default/css/styles.css",plugins:["advlist autolink link image lists charmap preview hr anchor","searchreplace wordcount visualblocks visualchars code fullscreen media nonbreaking","save table contextmenu directionality paste textcolor template imagetools importcss"],importcss_append:!0,importcss_groups:[{title:"Other styles"}],paste_as_text:!0,toolbar:"undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image anchor | print preview media fullpage | forecolor backcolor | code",image_advtab:!0,removed_menuitems:"newdocument",templates:[{title:"download",content:"[[download]]editme[[/download]]"},{title:"fig",content:'[[fig path="./sites/default/images/"]]caption[[/fig]]'},{title:"gallery",content:"[[gallery]]editme[[/gallery]]"},{title:"link",content:"[[link]]editme[[/link]]"},{title:"map",content:"[[map]]editme[[/map]]"},{title:"userform",content:"[[userform]]editme[[/userform]]"},{title:"vimeo",content:"[[vimeo]]editme[[/vimeo]]"},{title:"youtube",content:"[[youtube]]editme[[/youtube]]"}]};$.extend(a,e),tinymce.init(a)},preSave:function(){},setValue:function(t,e){tinymce.get(t).setContent(e)}},removeNotValid:function(t,e){e=e||{};var a="_,=.\\s\\?'\"\\-$:;’°“”";e.permit&&$.each(e.permit,function(t,e){a=a.replace(e,"")}),t.on("keyup",function(t){if(13!==t.keyCode&&37!==t.keyCode&&38!==t.keyCode&&39!==t.keyCode&&40!==t.keyCode&&8!==t.keyCode&&46!==t.keyCode&&16!==t.keyCode&&17!==t.keyCode&&18!==t.keyCode&&91!==t.keyCode&&93!==t.keyCode){var s=$(this).val();if(s&&""!==s){var i=s.replace(new RegExp("(["+a+"])","gi"),e.replace?e.replace:"");i=e.toLower?i.toLowerCase():i,$(this).val(i)}}}),e.replace&&t.on("change",function(){var t=$(this).val();t=t.replace(new RegExp(e.replace+"+","gi"),e.replace).replace(new RegExp(e.replace+"$"),"").replace(new RegExp("^"+e.replace),""),$(this).val(t)})},upload:function(t,e,a){if($("#qq-template").length<1){var s=$("<script>").attr("type","text/template").attr("id","qq-template").appendTo("body");s.html('<div class="qq-uploader-selector qq-uploader" qq-drop-area-text="Drop files here">    <div class="qq-total-progress-bar-container-selector qq-total-progress-bar-container">        <div role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" class="qq-total-progress-bar-selector qq-progress-bar qq-total-progress-bar"></div>    </div>    <div class="qq-upload-drop-area-selector qq-upload-drop-area" qq-hide-dropzone>        <span class="qq-upload-drop-area-text-selector"></span>    </div>    <div class="qq-upload-button-selector qq-upload-button">        <div>Upload a file</div>    </div>        <span class="qq-drop-processing-selector qq-drop-processing">            <span>Processing dropped files...</span>            <span class="qq-drop-processing-spinner-selector qq-drop-processing-spinner"></span>        </span>    <ul class="qq-upload-list-selector qq-upload-list" aria-live="polite" aria-relevant="additions removals">        <li>            <div class="qq-progress-bar-container-selector">                <div role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" class="qq-progress-bar-selector qq-progress-bar"></div>            </div>            <span class="qq-upload-spinner-selector qq-upload-spinner"></span>            <span class="qq-upload-file-selector qq-upload-file"></span>            <span class="qq-edit-filename-icon-selector qq-edit-filename-icon" aria-label="Edit filename"></span>            <input class="qq-edit-filename-selector qq-edit-filename" tabindex="0" type="text">            <span class="qq-upload-size-selector qq-upload-size"></span>            <button type="button" class="qq-btn qq-upload-cancel-selector qq-upload-cancel">Cancel</button>            <button type="button" class="qq-btn qq-upload-retry-selector qq-upload-retry">Retry</button>            <button type="button" class="qq-btn qq-upload-delete-selector qq-upload-delete">Delete</button>            <span role="status" class="qq-upload-status-text-selector qq-upload-status-text"></span>        </li>    </ul>    <dialog class="qq-alert-dialog-selector">        <div class="qq-dialog-message-selector"></div>        <div class="qq-dialog-buttons">            <button type="button" class="qq-cancel-button-selector">Close</button>        </div>    </dialog>    <dialog class="qq-confirm-dialog-selector">        <div class="qq-dialog-message-selector"></div>        <div class="qq-dialog-buttons">            <button type="button" class="qq-cancel-button-selector">No</button>            <button type="button" class="qq-ok-button-selector">Yes</button>        </div>    </dialog>    <dialog class="qq-prompt-dialog-selector">        <div class="qq-dialog-message-selector"></div>        <input type="text">        <div class="qq-dialog-buttons">            <button type="button" class="qq-cancel-button-selector">Cancel</button>            <button type="button" class="qq-ok-button-selector">Ok</button>        </div>    </dialog></div>')}a=a||{},new qq.FineUploader({request:{endpoint:e},element:t,validation:{allowedExtensions:a.allowedExtensions?a.allowedExtensions:[],sizeLimit:a.sizeLimit?a.sizeLimit:0,minSizeLimit:a.minSizeLimit?a.minSizeLimit:0},multiple:a.one?!1:!0,callbacks:{onComplete:function(t,e,s){return a.loaded?a.loaded(t,e,s):!1},onAllComplete:function(t,e){return a.allLoaded?a.allLoaded(t,e):!1}}})},filterList:function(t,e,a){t&&""!==t?a?(e.find(".search-text:not(:Contains("+t+"))").parents(".search-container").fadeOut(),e.find(".search-text:Contains("+t+")").parents(".search-container").fadeOut()):(e.find(".search-text:not(:Contains("+t+"))").fadeOut(),e.find(".search-text:Contains("+t+")").fadeIn()):e.find("."+(a?"search-container":"search-text")).fadeIn()},dialog:function(t){$("#modal").length>0&&$("#modal").modal("hide");var e=$("<div />").attr("id","modal").addClass("modal fade").append('<div class="modal-dialog"><div class="modal-content">'+(t.title?'<div class="modal-header"><h4>'+t.title+"</h4></div>":"")+"</div></div>").appendTo("body");t.width&&e.css({width:t.width,"margin-left":"-"+t.width/2+"px"});var a=$("<div />").addClass("modal-body").appendTo(e.find("div.modal-content"));if(t.buttons&&"object"==typeof t.buttons){var s=$("<div />").addClass("modal-footer").appendTo(e.find("div.modal-content"));$.each(t.buttons,function(t,a){var i=$("<a />").addClass("btn"+(a.addclass?" "+a.addclass:" btn-primary")).html((a.glyphicon?'<i class="glyphicon glyphicon-'+a.glyphicon+'"></i> ':"")+a.text);a.href&&i.attr("href",a.href),a.click&&("close"===a.click?i.attr("data-dismiss","modal"):i.click(function(){a.click(e)})),"close"===a.action&&i.attr("data-dismiss","modal"),i.appendTo(s)})}e.modal({keyboard:!0}),e.on("hidden.bs.modal",function(){$("body").removeClass("modal-open"),e.remove()}),t.html?(a.html(t.html),t.loaded&&t.loaded(a)):t.obj&&t.method&&(e.find(".modal-body").html('<i class="icon ion-load-c ion-loading-c" style="font-size: 3em;"></i>'),$.ajax({type:t.post?"POST":"GET",url:"controller.php?obj="+t.obj+"&method="+t.method+(t.param?"&param[]="+t.param.join("&param[]="):""),data:t.post}).done(function(a){e.find(".modal-body").html(a),t.loaded&&t.loaded(e)}).fail(function(){e.find(".modal-body").html('<p class="text-danger">'+admin.tr("error_check_log")+"</p>")}))},message:function(t,e,a,s){new PNotify({title:a?a:!1,text:t,type:e?e:!1,hide:s?!1:!0,styling:"bootstrap3"})},tabs:{tab:"",set:function(t){"string"==typeof t?this.tab=$(t):this.tab=t},start:function(){tab=this.tab,tab.find("a").click(function(t){t.preventDefault(),$(this).tab("show")}),tab.find("button.close").click(function(t){var e=$(this).parents("li");return admin.tabs.closeTab(e),!1})},add:function(t){var e=t.title?t.title:"",a=this.tab,s=Math.floor(1e3*Math.random())+""+a.find("li").length;if(this.tab.append('<li><a href="#added'+s+'">'+e+'<button class="close" type="button">×</button></a></li>'),t.html)this.tab.next("div.tab-content").append('<div class="tab-pane" id="added'+s+'">'+t.html+"</div>"),this.start(a),this.tab.find("li a:last").tab("show"),this.tab.find("li:last").data("url",location.hash.substring(1)),this.tab.find("li:last").data("opts",t);else{if(!t.obj||!t.method)return!1;admin.tabs.tab.next("div.tab-content").append('<div class="tab-pane" id="added'+s+'"><i class="icon ion-load-c ion-loading-c" style="font-size: 3em;"></i></div>'),admin.tabs.start(a),admin.tabs.tab.find("li a:last").tab("show"),this.tab.find("li:last").data("url",location.hash.substring(1)),this.tab.find("li:last").data("opts",t),$.ajax({type:t.post?"POST":"GET",url:"controller.php?obj="+t.obj+"&method="+t.method+(t.param?"&param[]="+t.param.join("&param[]="):""),data:t.post}).done(function(t){$("#added"+s).html(t)}).fail(function(){$("#added"+s).html('<p class="text-danger">'+admin.tr("error_check_log")+"</p>")})}this.tab.find("li:last").on("click",function(){_ignorehash=!0,location.hash="#"+$(this).data("url")})},reloadActive:function(){this.reload()},reloadThis:function(t){t instanceof jQuery||(t=$(t));var e=t.parents(".tab-pane").index();e&&this.reload(e)},reload:function(t){var e,a;t?(e=$("div.tab-content .tab-pane").get(t),e=$(e),a=$(tab.find("li").get(t)).data("opts")):(e=$("div.tab-content div.active"),a=tab.find("li.active").data("opts"));var s="controller.php?obj="+a.obj+"&method="+a.method+(a.param?"&param[]="+a.param.join("&param[]="):"");e.html('<i class="icon ion-load-c ion-loading-c" style="font-size: 3em;"></i>'),$.ajax({url:s,data:a.data}).done(function(t){e.html(t)}).fail(function(){e.html('<p class="text-danger">'+admin.tr("error_check_log")+"</p>")})},closeActive:function(t){var e=tab.find("li.active");admin.tabs.closeTab(e,t)},closeTab:function(t,e){if(1===tab.find("li").length)return void(e&&(location.hash="#"+e));if(t.hasClass("active"))if(e){var a=location.hash.substr(1);if(a===e)return void admin.tabs.reloadActive();location.hash="#"+e,tab.find("li a:last").tab("show"),$("#"+t.find("a").attr("href").replace("#","")).remove(),t.remove()}else{$("#"+t.find("a").attr("href").replace("#","")).remove(),t.remove(),tab.find("li a:last").tab("show");var s=tab.find("li:last").data("url");s&&"undefined"!==s?(_ignorehash=!0,location.hash="#"+s):location.hash="#"}else t.remove()}},tr:function(t){return lang[t]?lang[t]:t},media:{go2dir:function(t){admin.tabs.closeActive("media/all/"+t)},deleteDir:function(t){$.get("controller.php?obj=media_ctrl&method=delete&param[]="+t,function(t){admin.message(t.text,t.status),"success"==t.status&&admin.media.go2dir(t.new_path)},"json")},deleteFile:function(t){admin.dialog({title:admin.tr("pay_attention_please"),html:admin.tr("confirm_delete_file"),buttons:[{text:'<i class="glyphicon glyphicon-trash"></i>  '+admin.tr("delete"),addclass:"btn-danger",action:"close",click:function(){$.get("controller.php?obj=media_ctrl&method=delete&param[]="+t,function(t){admin.message(t.text,t.status),"success"==t.status&&admin.tabs.closeActive("media/all/"+t.new_path)},"json")}},{text:admin.tr("close"),action:"close"}]})},filterDir:function(t,e){t?($("#media-"+e+" div.searcheable:not(:Contains("+t+"))").parents("li").fadeOut(),$("#media-"+e+" div.searcheable:Contains("+t+")").parents("li").fadeIn()):$("#media-"+e+" li").fadeIn()}}};$(function(){admin.tabs.set("#tabs"),admin.tabs.start(),$(window).on("hashchange",function(t){if(_ignorehash)return void(_ignorehash=!1);var e=location.hash.substring(1);if(e&&!e.match(/nt-/)){var a=e.split("/");admin.tabs.add({title:a[0]+"/"+a[1]+(a[2]?"/"+a[2].substr(0,5)+"...":""),obj:a[0]+"_ctrl",method:a[1],param:a.slice(2)})}}).trigger("hashchange")}),jQuery.expr[":"].Contains=function(t,e,a){return(t.textContent||t.innerText||"").toUpperCase().indexOf(a[3].toUpperCase())>=0},$("a.ftpopover").popover({html:!0}),$(document).ready(function(){$(".toggleMenu").on("click",function(t){$("#menu, #content").toggleClass("shown"),"#"===$(this).attr("href")&&t.preventDefault()}),$("#content").on("click",function(){$("#menu, #content").removeClass("shown"),$("#menu ul.sub").removeClass("show")}),$("#menu a").on("click",function(){"#"!==$(this).attr("href")&&($("#menu ul.sub").removeClass("show"),$("#menu, #content").removeClass("shown"))}),$("#menu a.sub-toggle").on("click",function(t){return $(this).next("ul.sub").toggleClass("show"),$("#menu ul.sub").not($(this).next("ul.sub")).removeClass("show"),t.preventDefault(),!1}),"ontouchstart"in document.documentElement||$('[data-toggle="tooltip"]').tooltip()});